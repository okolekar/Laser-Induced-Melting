...
def Update_Crys(T=None,H=None,run=None):
    ...
    #If T is provided then the below if statement is activated and the H vector is formulated
    if T is not None:
        return np.where(T < 0, ip.D*T, np.where(T == 0,0, ip.D*T+ip.D*ip.lambf))
    #else if H is provided then the below elif statement is activated and the T vector is formulated
    elif H is not None:
        return np.where(H < 0, H/ip.D, np.where(H < ip.lambf,0, (H-ip.D*ip.lambf)/ip.D))
    else:
