...
'''
class Mesh():
    def __init__(self,s = 0,nodes = ip.n,length = ip.length):
        self.nl         = self.mesh_list(s,nodes,length)
        self.phi        = None
        self.dphi       = None
        self.J          = None
        self.h          = self.nl[1]-self.nl[0]
        if self.h == 0 or self.h <0:
            raise ValueError(f"The mesh inverted. Element edge length = {self.h}")
        self.run_count  = 0
    '''
#############################################################################################################################
Method test_phi(): -
...
#############################################################################################################################
'''
    def mesh_list(self,s,nodes,length):
        l = []
        meshlength = length - s
        for q in range(nodes):
            l.append(s+q*meshlength/(nodes-1))
        if s == 0 :
            if round(l[-1],5) != round(meshlength,5) or len(l) == 0:
                print(f"\nError in Mesh module, the mesh list {l} is not generated correctly for s = {meshlength}")
        else:
