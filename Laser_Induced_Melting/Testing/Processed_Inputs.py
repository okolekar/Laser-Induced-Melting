...
'''
if mat_type == 1:
    Ta = -0.4#(293-ip.meltpt)/(ip.vapourpt-ip.meltpt)             
    D = ip.rho*ip.c*(ip.vapourpt-ip.meltpt)/((ip.rho*ip.c*ip.vapourpt+ip.Lf*ip.rho)-(ip.rho*ip.c*ip.meltpt))#0.35#alpha#ip.rho*ip.c*(ip.vapourpt-ip.meltpt)/((ip.rho*ip.c*ip.vapourpt+ip.Lf*ip.rho)-(ip.rho*ip.c*ip.meltpt))
...
if mat_type == 2:
     ...
     Hliq = D*Tliq + D*lambf
     length = 10#ip.Iref/(ip.k*(ip.vapourpt-ip.soliduspt))
     K = 2.5
#===========================================================================================================================#
